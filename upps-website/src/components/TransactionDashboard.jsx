...
import BillRequest from "./BillRequest.jsx";
import PaymentSlipRequest from "./PaymentSlipRequest.jsx"
import { BillDetailsModal } from "./BillDetailsModal.jsx";
import { PaymentSlipPrintDetails } from "./PaymentSlipPrintDetails.jsx"
...
export const TransactionDashboard = () => {
  ...
  const generateBill = (requestDetail, requesttype) => {
    if (requesttype === "Examination") {
      ...
      const url = '/officehead/joborderform'; // URL of the payment slip
      console.log("Selected Queue Request:", requestDetail);
  ...
  const fetchBillRequest = async () => {
    try {
      ...
      const printingBill = billdata.filter((request)  => request.request_details.service_type.service_type_name === "Printing")
      ...
      setRequestTypes([
        ...new Set(
          printingBill.map(
            (item) =>
              item.request_details.print_request_details.printing_type
                .printing_type_name
          )
        ),
      ]);
      setJobOrderTypes([...new Set(printingBill.map((item) => item.type))]);

      const mappedData = printingBill.map((requestDetail) =>
        BillRequest({
          requestDetail,
          handlePaidStatus,
          handleDetailsClick,
  ...
  const fetchPaymentSlip = async () => {
    try{
      ...
      const printingPaymentSlip = data.filter((request)  => request.request_details.service_type.service_type_name === "Printing" && request.paid_status === "Paid")

      const mappedPSPrint = printingPaymentSlip.map((requestDetail) => PaymentSlipRequest({
        requestDetail,
