...
import BillBookBindRequest from "./BillBookBindRequest.jsx";
import PaymentSlipBookBindRequest from "./PaymentSlipBookBindRequest.jsx"
import { BillBookBindDetails } from "./BillBookBindDetails.jsx";
import { PaymentSlipBookBindDetails } from "./PaymentSlipBookBindDetails.jsx"
import { FilterRequest } from "./FilterRequest.jsx";
import { BillingForm } from "../screens/BillingForm.jsx";
import { useNavigate } from "react-router-dom";

export const BookBindingTransactions = () => {
  ...
  const generateBill = (requestDetail) => {
    const url = '/officehead/joborderbookbindform'; // URL of the payment slip
  ...
  const fetchBillRequest = async () => {
    try {
      ...
      const bookbindBill = billdata.filter((request)  => request.request_details.service_type.service_type_name === "Book Binding")
      ...
      setRequestTypes([
        ...new Set(
            bookbindBill.map(
            (item) =>
              item.request_details.book_binding_request_details.request_type
                .request_type_name
          )
        ),
      ]);
      setJobOrderTypes([...new Set(bookbindBill.map((item) => item.type))]);

      const mappedData = bookbindBill.map((requestDetail) =>
        BillBookBindRequest({
          requestDetail,
          handlePaidStatus,
          handleDetailsClick,
  ...
  const fetchPaymentSlip = async () => {
    try{
      ...
      const bookBindPaymentSlip = data.filter((request)  => request.request_details.service_type.service_type_name === "Book Binding")

      

      const mappedPSPrint = bookBindPaymentSlip.map((requestDetail) => PaymentSlipBookBindRequest({
        requestDetail,
