      ...
      <div className="bg-[#ffffff] p-8 rounded-xl w-11/12 max-w-lg relative shadow-2xl border border-gray-700">
        ...
        <div className="grid grid-cols-2 gap-4 text-[#17153a]">
          ...
          <h1 className='text-xl'>Type</h1>
            <p className='text-base'>{bookbindRequest? bookbindRequest.service_type.service_type_name: 'N/A'}</p>
          </div>
          <div>
          ...
          {user.id === personnelRequest.id || user.role === "Office Head" ? ( 
            ...
            <h1 className='text-xl'>File</h1>
                <button onClick={() => window.open(requestData.book_bind_personnel_request.pdf, "_blank")} href={requestData.book_bind_personnel_request && requestData.book_bind_personnel_request.pdf ? requestData.book_bind_personnel_request.pdf : "#"} className="text-[#4d70f1] text-[1.1rem]   ">
                  {requestData.book_bind_personnel_request && requestData.book_bind_personnel_request.pdf ? "View File" : "N/A"}
                </button>
            </div>
           ) : ( 
            <div>
